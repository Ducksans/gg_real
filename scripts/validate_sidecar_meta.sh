...
# 제외: admin/templates/**, .git, .github(워크플로는 제외), node_modules

is_target() {
  case "$1" in
    ./admin/*|./apps/*|./scripts/*|./docs/*|./AGENTS.md|./basesettings.md|./SECURITY.md|./THREAT_MODEL.md) return 0 ;;
    *) return 1 ;;
  esac
}
...
while IFS= read -r -d '' f; do
  ...
  case "$f" in ./admin/templates/*|*/node_modules/*) continue ;; esac
  if needs_sidecar "$f"; then
    meta="$f.meta.yaml"
    check_meta "$meta" || err=1
  fi
