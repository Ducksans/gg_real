...
- 트리거 문구: 사용자가 “오늘은 여기까지”, “오늘 작업 마무리” 등 명시적으로 종료 의사를 밝히면 발동한다.
- 종료 모드에서는 새로운 실행/수정을 진행하지 않고 마무리 절차만 수행한다.
- 마무리 절차
  1. **요약 준비**: 당일 수행한 주요 작업, 수정한 파일, 남은 TODO를 정리한다. 문서/코드 경로를 명시하고 회의 모드 상태를 확인한다.
  2. **SSoT 업데이트**: `admin/checkpoints/`에 일일 요약 파일을 작성하거나 갱신하고, 관련 상태 문서(`admin/state/session.json` 등)에 세션 종료 기록이 필요한지 확인한다.
  3. **검증**: 열린 터미널/테스트가 있다면 `pnpm run validate:docs`, `pnpm run validate:refs` 등의 기본 검증을 실행해 깨끗한 상태를 보장한다.
  4. **깃 동기화 질의**: 변경 사항이 있다면 섹션 8 규칙에 따라 “깃 허브에 동기화 할까요?”를 확인한다. 사용자가 동의하면 커밋/푸시까지 수행하고, 동의가 없으면 스테이징만 해두고 대기한다.
  5. **최종 리포트**: 사용자에게 “오늘 작업 요약 / 테스트 결과 / 남은 과제 / 깃 동기화 현황”을 포함한 마무리 메시지를 전달한다.
  6. **리셋**: 로컬 개발 서버나 임시 자원(스크립트, 포트 등)을 종료하고 회의모드로 전환한 뒤 종료 사실을 기록한다.
- 위 절차 후 사용자가 추가 지시를 내리면 종료 모드를 해제하고 섹션 4 절차를 다시 따른다.
